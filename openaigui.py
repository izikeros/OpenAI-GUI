...
def modules(engines):
    return (
        engines
        if engines in models
        else ValueError(f"Invalid engine: {engines}. Must be one of {models}")
    )
...
def select_max_tokens(max_tokens):
    return (
        max_tokens
        if max_tokens in max_tokens_list
        else ValueError(
...
def picture_size(size):
    return (
        ...
        else ValueError(f"Invalid max_tokens: {size}. Must be one of {size_list}")
    )
...
def openDAN(prompt_inn):
    ...
    chat = openai.ChatCompletion.create(model="gpt-3.5-turbo", messages=api_message)
...
def dalle(prompt_ins, size):
    response = openai.Image.create(prompt=prompt_ins, n=1, size=picture_size(size))
                    ...
                    [
                        sg.Tab(
                            ...
                            [
                                ...
                                [
                                    ...
                                    sg.Combo(
                                        max_tokens_list,
                                        default_value=max_tokens_list[0],
                                        key="-MAXTOKENS-",
                                        readonly=True,
                                    ),
                                ],
                                [
                                    sg.Text(
                                        "Enter your question or statement below:",
                                        font=("_ 13"),
                                    )
                                ],
                                [
                        ...
                        sg.Tab(
                            ...
                            [
                                ...
                                [
                                    sg.Button("Dan answer", bind_return_key=True),
                                    sg.Button("Open file"),
                                    sg.Button("Clear"),
                                    sg.Button("Quit"),
                                ],
                        ...
                        sg.Tab(
                            ...
                            [
                                [
                                    sg.Text(
                                        "Suggest impression:", font=("Arial", 9, "bold")
                                    )
                                ],
                                [
                                    sg.Radio(
...
def main():
    ...
    while True:
        ...
        if values is not None:
            engines = (
                values["-ENGINES-"]
                if values["-ENGINES-"] == "Choose model"
                else values["-ENGINES-"]
            )
        if values is not None:
            max_tokens = (
                values["-MAXTOKENS-"]
                if values["-MAXTOKENS-"] == "Choose max token"
                else values["-MAXTOKENS-"]
            )
        if values is not None:
            size = (
                values["-SIZE-"]
                if values["-SIZE-"] == "Choose picture size"
                else values["-SIZE-"]
            )
        if event == "Answer":
